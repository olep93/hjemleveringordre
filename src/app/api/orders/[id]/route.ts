...
function buildTitle(orderNumber?: string | null, customerName?: string | null) {
  if (orderNumber && customerName) return `Kundeordre ${orderNumber} – ${customerName}`;
...
export async function GET(
  ...
  try {
    ...
    const photos = await Promise.all(
      (Array.isArray(data.photos) ? data.photos : []).map(
        async (photo: { path?: string; uploadedBy?: string; createdAt?: string }) => ({
          ...photo,
          url: await signedUrl(photo.path)
        })
      )
    );
    ...
    return NextResponse.json({
      order: {
        ...
        originalDocumentUrl: await signedUrl(data.originalDocumentPath),
        photos,
    ...
    const message = error instanceof Error ? error.message : "Kunne ikke hente ordre.";
    return NextResponse.json(
      { error: message },
...
export async function PATCH(
  ...
  try {
    ...
    const newOrderNumber =
      "orderNumber" in body ? body.orderNumber?.trim() || null : current.orderNumber ?? null;
    const newCustomerName =
      "customerName" in body ? body.customerName?.trim() || null : current.customerName ?? null;
    ...
    const message = error instanceof Error ? error.message : "Kunne ikke oppdatere ordre.";
    return NextResponse.json(
      { error: message },
      {
        status:
...
export async function POST(
  ...
  try {
    await requireRole(["EMPLOYEE", "MANAGER", "ADMIN"]);
    const { id } = await context.params;
    const body = (await request.json()) as { action?: string; actorName?: string };
    ...
    const [buffer] = await adminStorage
      .bucket()
      .file(data.originalDocumentPath)
      .download();
    ...
    await ref.update({
      ...
      status: data.status === "DEVIATION" && orderNumber ? "TO_PICK" : data.status,
      importError: null,
      updatedAt: FieldValue.serverTimestamp()
    });
    ...
    const message = error instanceof Error ? error.message : "Kunne ikke tolke dokumentet.";
    return NextResponse.json(
      { error: message },
...
export async function DELETE(
  ...
  try {
    ...
    if (!snapshot.exists) {
      return NextResponse.json({ error: "Ordren er allerede slettet." }, { status: 404 });
    }

    try {
      await adminStorage.bucket().deleteFiles({ prefix: `orders/${id}/` });
    } catch (storageError) {
      console.warn("Ordrefiler kunne ikke slettes, fortsetter med Firestore:", storageError);
    }
    ...
    const message = error instanceof Error ? error.message : "Kunne ikke slette ordren.";
