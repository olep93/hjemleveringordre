...
export async function POST(request: NextRequest) {
  try {
    ...
    let originalDocumentPath: string | null = null;
    let source = "MANUAL";

    if (file instanceof File && file.size > 0) {
      const buffer = Buffer.from(await file.arrayBuffer());
      const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, "_");
      originalDocumentPath = `orders/${orderRef.id}/manual-${safeName}`;
      await adminStorage.bucket().file(originalDocumentPath).save(buffer, {
        resumable: false,
    ...
    await orderRef.set({
      internalId: `HL-${new Date().getFullYear()}-${orderRef.id.slice(0, 8).toUpperCase()}`,
      title,
      orderNumber,
      customerName,
      phone,
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "Kunne ikke opprette ordre." },
      { status: 400 }
    );
  }
}
