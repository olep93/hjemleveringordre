  ...
  try {
    ...
    if (!pdfAttachment) {
      ...
      try {
        ...
        originalDocumentPath = `orders/${documentId}/original-${filename}`;
        await adminStorage.bucket().file(originalDocumentPath).save(pdfBuffer, {
          resumable: false,
    ...
    const hasCoreData = Boolean(orderNumber && customerName);
    const title = hasCoreData
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "Ukjent feil." },
      { status: 400 }
    );
  }
}
